...
export class Geometry {
  ...
  private _catmullClarkSubdivide(): void {
    ...
    for (let i = 0; i < indices.length; i += 3) {
      ...
      const facePoint = [(v0[0] + v1[0] + v2[0]) / 3, (v0[1] + v1[1] + v2[1]) / 3, (v0[2] + v1[2] + v2[2]) / 3];
    ...
    edgeMap.forEach((facePoints, key) => {
      ...
      const avgFacePoints = facePoints
        .reduce(
          (acc, fp) => {
            acc[0] += fp[0];
            acc[1] += fp[1];
    ...
    for (let i = 0; i < positions.length / 3; i++) {
      ...
      const avgEdgeMidpoint = connectedEdges
        .reduce(
          (acc, ep) => {
            acc[0] += ep[0];
            acc[1] += ep[1];
        ...
        .map((sum) => sum / connectedEdges.length);
