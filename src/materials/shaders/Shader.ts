    ...
    for (const [name, varying] of this.varyings) {
      if (varying.interpolate) {
        ...
        if (
          type === "flat" &&
          type &&
          !["first", "either"].includes(sampling as string)
        ) {
          varying.interpolate.sampling = "first";
          console.warn(
            `Invalid sampling mode for flat interpolation: ${varying.interpolate.sampling}. Using 'first' instead.`,
          );
        }
        if (
          type !== "flat" &&
          ...
          console.warn(
            `Invalid sampling mode for ${varying.interpolate.type} interpolation: ${varying.interpolate.sampling}. Using 'center' instead.`,
          );
        }
      } else {
        varying.interpolate = {
