...
class GLTFLoader {
  ...
  async parseScenes(gltf: any, buffers: any) {
    return await Promise.all(
      gltf.scenes.map(async (scene: any) => {
    ...
    try {
      if (primitive.material !== undefined) {
        ...
        if (gltfMaterial.occlusionTexture !== undefined) {
          const texture = await this.loadTexture(gltf, buffers, gltfMaterial.occlusionTexture.index);
          material.ao_map?.setTexture(texture);
        }
  ...
  async loadTexture(gltf: any, buffers: any, textureIndex: number) {
    ...
    const slice = buffer.arrayBuffer.slice(
      buffer.byteOffset + bufferView.byteOffset,
      buffer.byteOffset + bufferView.byteOffset + bufferView.byteLength,
    );
  ...
  parseAccessor(gltf: any, buffers: any, accessorIndex: any) {
    ...
    let array = new TypedArray(
      buffer.arrayBuffer.slice(byteOffset, byteOffset + count * numberOfComponents * TypedArray.BYTES_PER_ELEMENT),
    );
