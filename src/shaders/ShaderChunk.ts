...
export class ShaderChunk {
    ...
    extractBindings(layouts: BindGroupLayout[]): Binding[] {
        ...
        for (const layout of layouts) {
            for (const binding of layout.bindings) {
                let match;
                while ((match = bindingRe.exec(this.bindingCode)) !== null) {
                    if (
                        match[1] === binding.description.groupName &&
                        match[2] === binding.description.bindingName
                    ) {
    ...
    shouldInsert(stage: "vertex" | "fragment" | "compute") {
        return (
            this.bodyCodes.some((bodyCode) => bodyCode.type === stage) ||
            this.bodyCodes.length === 0
        );
    }

    hasOrderRules(stage: "vertex" | "fragment" | "compute") {
        return this.bodyCodes.some(
            (bodyCode) => bodyCode.type === stage && bodyCode.order,
        );
    }

    getBodyCodes(stage: "vertex" | "fragment" | "compute") {
        return this.bodyCodes.filter((bodyCode) => {
            return bodyCode.type === stage;
        });
    }

    getBodyCodesOrdered(stage: "vertex" | "fragment" | "compute") {
        return this.bodyCodes.filter(
            (bodyCode) => bodyCode.type === stage && bodyCode.order,
        );
    }
    ...
    extractCode() {
        ...
        while ((match = bodyRe.exec(this.template)) !== null) {
            if (match[1] === "fragment") {
                this.bodyCodes.push(
                    new BodyCode(match[3], match[2] || "", "fragment"),
                );
            }
            if (match[1] === "vertex") {
                this.bodyCodes.push(
                    new BodyCode(match[3], match[2] || "", "vertex"),
                );
            }
            if (match[1] === "compute") {
                this.bodyCodes.push(
                    new BodyCode(match[3], match[2] || "", "compute"),
                );
            }
        }
        this.defines = this.template
            .replace(includeRe, "")
            .replace(bodyRe, "")
            .trim();
    ...
    get isCompute() {
        return this.bodyCodes.some((bodyCode) => bodyCode.type === "compute");
    }

    get isFragment() {
        return this.bodyCodes.some((bodyCode) => bodyCode.type === "fragment");
    }

    get isVertex() {
        return this.bodyCodes.some((bodyCode) => bodyCode.type === "vertex");
    }
}
