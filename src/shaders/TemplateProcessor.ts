...
export class TemplateProcessor {
    ...
    static processTemplate(
        ...
        chunks: ShaderChunk[] = [],
        defines: Map<string, boolean> = new Map(),
    ) {
    ...
    private processChunk = (chunk: ShaderChunk, layouts: BindGroupLayout[]) => {
        ...
        for (const subChunk of chunk.chunks) {
            const shaderChunk = ShaderLibrary.getChunk(subChunk);
            if (shaderChunk) {
                this.processChunk(shaderChunk, layouts);
            }
        }
    };
    ...
    processMain(template: string) {
        ...
        for (const binding of sortedBindings) {
            ...
            if (struct && !bindingsStr.includes(struct.name)) {
                wgsl = `\n${struct.toWGSL()}\n${wgsl}\n`;
            }
            bindingsStr += `${wgsl}\n`;
        }
        ...
        const body = chunks
            .map((chunk) => chunk.getBodyCodesUnordered(this.type))
            .flat()
            .map((bodyCode) => bodyCode.code);
        let bodyStr = body.join("\n");

        // sort ordered chunks
        const orderedBodyCodes = chunks
            .map((chunk) => chunk.getBodyCodesOrdered(this.type))
            .flat()
            .sort((a, b) => {
                const aRule = a.order;
                const bRule = b.order;
        ...
        for (const bodyCode of orderedBodyCodes) {
            ...
            if (rule.includes("before")) {
                ...
                if (index !== -1) {
                    console.warn("found marker")
                    splitBody.splice(index, 0, bodyCode.code);
                } else {
                    console.warn("not found marker", marker, bodyStr, template);
                }
            ...
            if (rule.includes("after")) {
                ...
                const index = splitBody.findIndex((line) => new RegExp(`// ?${marker}`).test(line));
                if (index !== -1) {
                    console.warn("found marker", marker)
                    // find next marker
                    const nextMarkerIndex =
                        splitBody.slice(index + 1).findIndex((line) => /\/\/\s?\w+/.test(line)) + (index + 1);
                    ...
                    console.error("not found marker", marker);
                }
            }
        }
