...
class Shader {
  ...
  static DEFAULTS = {
    ...
    FRAGMENT: `
        ...
        `,
    COMPUTE: `
            @compute(input) @workgroup_size(1, 1, 1) {
                {{compute}}
            }
        `,

    OUTPUTS: [{ location: 0, name: "color", type: "vec4f" }],
  ...
  BUILTIN_VERTEX_ATTRIBUTES = [{ name: "vertex_index", type: "u32" }];
  ...
  constructor(options: ShaderConfig) {
    ...
    this.isCompute = !!options.compute || Array.from(this.chunks).some(c => c.isCompute);
  ...
  generateVaryings() {
    ...
    for (const [name, varying] of this.varyings) {
        ...
        if (varying.type === "u32" || varying.type === "i32") {
            varying.interpolate = {
                type: "flat",
                sampling: "first",
            };
        ...
        let location = index++;
        result += `@location(${location}) @interpolate(${varying.interpolate?.type || "perspective"}, ${varying.interpolate?.sampling || "center"}) ${name}: ${varying.type},\n`;
    }
    return result;
  }
  ...
  setAttributes(attributes: ShaderAttribute[]) {
    ...
    this.vertexSource = this.vertexSource.replace(
      /struct VertexInput {[\s\S]*?};/,
      "",
    );
