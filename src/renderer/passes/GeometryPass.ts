...
export class GeometryPass extends RenderPass {
    ...
    private createTextures() {
        ...
        this.width = this.renderer.width;
        this.height = this.renderer.height;
        this.positionTexture = this.resources.createTexture(
            "position_texture",
            {
        ...
        this.depthTexture = this.resources.getTexture("depth_texture")!;
        ...
        this.renderPassDescriptor = {
            label: "GeometryPassRenderPassDescriptor",
            colorAttachments: [
                {
    ...
    createRenderable(mesh: Mesh, camera: Camera) {
        ...
        const bindGroups = [
            this.resources.createBindGroup(this.layouts[0], {
                Scene:
                    mesh.scene?.uniforms.get("Scene") ||
                    UniformData.getByName("Scene")!,
                Camera: camera.uniforms.get("Camera")!,
            }),
            this.resources.createBindGroup(this.layouts[1], {
                MeshInstances:
                    renderable.visibilityInfo?.visibleInstancesBuffer ||
                    mesh.uniforms.get("MeshInstances")!,
            }),
        ...
        const pipelineLayout = this.pipelines.createPipelineLayout(
            this.layouts,
        );
    ...
    draw(object: Object3D, camera: Camera, pass: GPURenderPassEncoder) {
        if (object instanceof Mesh) {
            // if (object.material.renderState.transparent) {
            //     return;
            // }
            if (object.isCulled && !object.isInstanced) {
                if (!camera.frustum.intersectsObject(object)) {
                    return;
                }
            }
            let renderable =
                this.renderables.get(object) ||
                this.createRenderable(object, camera);
