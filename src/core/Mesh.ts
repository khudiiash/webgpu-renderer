...
import { BoundingSphere, Euler, Quaternion, Vector3 } from "@/math";
import { Struct } from "@/data/Struct";
import { ShaderChunk } from "@/shaders/ShaderChunk";
import { ShaderLibrary, ShaderVarying } from "@/renderer";
import { BVH } from "@/data/BVH";
import { GPUBaseType } from "@/types";
...
export class Mesh extends Object3D {
    ...
    constructor(
        ...
        this.isCulled = options.isCulled ?? true;
        ...
        this.varyings = [
            ...
            { name: "local_position", type: "vec3f" },
        ...
        this.uniforms.set(
            ...
            new UniformData(this, {
                name: "BoundingSphere",
                isGlobal: false,
                struct: BoundingSphere.struct,
                values: {
                    center: this.geometry.boundingSphere.center,
    ...
    set count(value: number) {
        ...
        this.uniforms.delete("MeshInstances");
        this.uniforms.set(
            "MeshInstances",
            new UniformData(this, {
                name: "MeshInstances",
    ...
    addVarying(name: string, type: GPUBaseType) {
        this.material.shader.addVarying({ name, type });
    }
    ...
    initializeSpatialGrid(gridSize: number = 10) {
        ...
        for (let i = 0; i < indices.length; i += 3) {
            ...
            let startCellX = Math.floor(
                (triMinX - this.bounds.minX) * invCellSizeX,
            );
            let startCellZ = Math.floor(
                (triMinZ - this.bounds.minZ) * invCellSizeZ,
            );
            let endCellX = Math.floor(
                (triMaxX - this.bounds.minX) * invCellSizeX,
            );
            let endCellZ = Math.floor(
                (triMaxZ - this.bounds.minZ) * invCellSizeZ,
            );
    ...
    setAllRotations(rotations: ArrayLike<number>) {
        for (let i = 0; i < this.count; i++) {
            ...
            _mat.compose(
                position,
                Quaternion.instance.setFromEuler(euler),
                scale,
            );
            this.localInstanceMatrices.setSilent(_mat, i * 16);
        }
    ...
    private updateInstanceWorldMatrix(index: number) {
        ...
        const worldMatrix = Matrix4.instance.multiplyMatrices(
            this.matrixWorld,
            localMatrix,
        );
    ...
    addChunk(chunk: string | ShaderChunk): this {
        if (typeof chunk === "string") {
            ...
            if (!shaderChunk) {
                throw new Error(
                    `No chunk with name "${chunk}" found in the Shader Library`,
                );
            }
    ...
    clone() {
        const newMesh = new Mesh(this.geometry, this.material, {
            count: this.count,
            useBillboard: this.useBillboard,
            castShadow: this.castShadow,
        });

        newMesh.chunks = this.chunks;
        return newMesh;
    }
}
