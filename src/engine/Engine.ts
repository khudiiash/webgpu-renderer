...
import { Plane, Vector2, Vector3 } from '@/math';
...
import { Texture, Texture2D } from '@/data';
...
export class Engine extends EventEmitter {
    ...
    async init() {
        ...
        scene.ambientColor.set([0.2, 0.2, 0.2, 1]);
        ...
        scene.ambientColor.set([0.5, 0.5, 0.5, 1]);
        const name = 'lava';


        const mat = new StandardMaterial({ 
            diffuse_map: Texture2D.from(`assets/textures/${name}/${name}_albedo.jpg`),
            ao_map: Texture2D.from(`assets/textures/${name}/${name}_ao.jpg`),
            height_map: Texture2D.from(`assets/textures/${name}/${name}_height.png`),
            normal_map: Texture2D.from(`assets/textures/${name}/${name}_normal.png`),
            roughness_map: Texture2D.from(`assets/textures/${name}/${name}_roughness.jpg`),
            emissive_map: Texture2D.from(`assets/textures/${name}/${name}_emissive.jpg`),
            //uvScale: new Vector2(30, 30)
        });

        const plane = new Mesh(new BoxGeometry(10, 10, 10), mat);
        //plane.rotation.x = Math.PI / 2;
        scene.add(plane)
        mat.addChunk(new ShaderChunk('lava', `
            @include(Noise)
            ...
            @fragment(after:declares) {{
                ...
                let noiseSpeed = 0.5;
                let noiseScale = 0.5;
                var coord = input.vPositionW.xy;
                let noise = perlinNoise(coord * noiseScale + scene.time * noiseSpeed) * 0.5 + 0.5;
                uv *= noise * noiseStrength + 1.0;
                emissive_factor = sin(scene.time * noiseSpeed * noise * noiseStrength) + 1.0;
            }}
        `))

        const point = new PointLight({ intensity: 5000, range: 100, color: '#ffffff' });
        const bulb = new Mesh(new SphereGeometry(0.5), new StandardMaterial({ emissive: '#ffffff', emissive_factor: 10 }));
        point.add(bulb);
        scene.add(point);
        camera.setPosition(30, 30, 20);
